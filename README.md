...
- Run `npm run build` (or simply start the dev server with `npm run dev`) to regenerate the PWA manifest, service worker cache version, and the autogenerated `src/config/pwaVersion.ts` metadata file.
- Do not edit `public/manifest.json` or `src/config/pwaVersion.ts` directly; both are generated by `scripts/generate-manifest.mjs`.
