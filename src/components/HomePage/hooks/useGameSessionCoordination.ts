...
export function useGameSessionCoordination({
  ...
  const initialGameSessionData: GameSessionState = {
    ...
    gameType: initialState.gameType ?? 'soccer',
  ...
  const buildGameSessionHistorySlice = useCallback((state: GameSessionState) => {
    const slice = {
      ...
      gameNotesAiMeta: state.gameNotesAiMeta,
  ...
  const handleFieldNumberChange = useCallback((value: string) => {
  ...
  const handleTeamIdChange = useCallback((teamId: string | undefined) => {
    dispatchGameSession({ type: 'SET_TEAM_ID', payload: teamId });
  }, [dispatchGameSession]);
  ...
  const handleSetGameType = useCallback((newGameType: import('@/types').GameType) => {
    logger.log('[useGameSessionCoordination] handleSetGameType called with:', newGameType);
