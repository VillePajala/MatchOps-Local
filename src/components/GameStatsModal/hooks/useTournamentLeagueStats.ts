/**
 * Hook for calculating tournament and league statistics
...
interface UseTournamentLeagueStatsParams {
  activeTab: StatsTab;
...
export function useTournamentLeagueStats(
  params: UseTournamentLeagueStatsParams
      ...
      if (activeTab === 'season') {
        if (selectedSeasonIdFilter === 'all') {
          ...
          gameIds.forEach(gameId => {
            ...
            const season = leagues.find(s => s.id === game.seasonId);
            if (!season) return;
          ...
          const season = leagues.find(s => s.id === selectedSeasonIdFilter);
          if (!season) return [];
  ...
  }, [activeTab, savedGames, leagues, tournaments, selectedSeasonIdFilter, selectedTournamentIdFilter]);
}
