      ...
      <div className="relative z-10 flex-1 flex flex-col px-6 py-8 pb-safe">
        ...
        <div className={`flex-1 flex flex-col ${isFirstTimeUser ? 'justify-start pt-[20vh]' : 'justify-center'}`}>
          ...
          <div className="max-w-sm mx-auto w-full space-y-3">
            ...
            {/* App guide - subtle text link, available to all users so the
                in-app guide is reachable from the start screen (not just via
                Settings → Reset App Guide). */}
            <button
              type="button"
              onClick={() => setIsInstructionsModalOpen(true)}
              className="w-full py-3 flex items-center justify-center gap-1.5 text-slate-400 hover:text-slate-200 transition-all"
            >
