...
export class LocalDataStore implements DataStore {
  ...
  async createGame(game: Partial<AppState>): Promise<{ gameId: string; gameData: AppState }> {
    ...
    // Defaults-then-spread pattern: provide defaults for absent keys,
    // then override with any values provided in the game partial.
  ...
  async upsertPlanningSession(
    ...
    return withKeyLock(PLANNING_SESSIONS_KEY, async () => {
      ...
      // re-clone). Scalar string/boolean/number fields like
      // `parentSessionId` ride along safely via the spread; array
      // and object fields like `gameIds`, `includedGameIds`, `draft`
      // need explicit clone treatment to prevent shared-reference
      // mutation hazards (see those entries below).
      const normalized: PlanningSession = {
        ...session,
        name: session.name.trim(),
        gameIds: [...session.gameIds],
