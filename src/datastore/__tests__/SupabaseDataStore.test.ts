...
describe('SupabaseDataStore', () => {
  ...
  describe('Game Transforms', () => {
    ...
    describe('transformGameToTables', () => {
      ...
      /**
       * @critical - Rule 1: an empty string must reach the column as NULL, or
       * "no captain" is stored as a player id that matches nobody.
       */
      it('writes captain_id, with empty and absent both becoming NULL', () => {
        const transformGameToTables = getPrivateMethod('transformGameToTables');
        const base = {
          teamName: 'T', opponentName: 'O', gameDate: '2024-01-15', homeOrAway: 'home' as const,
          numberOfPeriods: 2 as const, periodDurationMinutes: 10, currentPeriod: 1, gameStatus: 'notStarted' as const,
        ...
        expect(transformGameToTables('g1', base, 'u1').game.captain_id).toBeNull();
        expect(transformGameToTables('g1', { ...base, captainId: '' }, 'u1').game.captain_id).toBeNull();
        expect(transformGameToTables('g1', { ...base, captainId: 'p7' }, 'u1').game.captain_id).toBe('p7');
      });
    ...
    describe('transformTablesToGame', () => {
      ...
      it('reads captain_id back, and a NULL as no captain rather than an empty id', () => {
        const transformTablesToGame = getPrivateMethod('transformTablesToGame');
        const row = {
          id: 'game_123', user_id: 'user_123', season_id: null, tournament_id: null,
          team_name: 'T', opponent_name: 'O', game_date: '2024-01-15', home_or_away: 'home',
          number_of_periods: 2, period_duration_minutes: 10, current_period: 1, game_status: 'notStarted',
          is_played: true, home_score: 0, away_score: 0, game_notes: '', show_player_names: true,
          created_at: '2024-01-15T10:00:00Z', updated_at: '2024-01-15T10:00:00Z',
        };
        const rest = { players: [], events: [], assessments: [], tacticalData: null };
        expect(transformTablesToGame({ game: { ...row, captain_id: 'p7' }, ...rest }).captainId).toBe('p7');
        // Legacy rows have no column at all; neither they nor a NULL may
        // produce an empty-string id that no player will ever match.
        expect(transformTablesToGame({ game: { ...row, captain_id: null }, ...rest }).captainId).toBeUndefined();
        expect(transformTablesToGame({ game: row, ...rest }).captainId).toBeUndefined();
      });
  ...
  describe('clearAllUserData', () => {
    ...
    it('should throw NetworkError when RPC function missing (PGRST202) - no fallback for atomicity', async () => {
      ...
      // No fallback to manual deletion - atomicity requires RPC
      // See Issue #332 for rationale
      await expect(dataStore.clearAllUserData()).rejects.toThrow(NetworkError);
      await expect(dataStore.clearAllUserData()).rejects.toThrow('temporarily unavailable');
    });

    it('should throw NetworkError when RPC permission denied - no fallback for atomicity', async () => {
      // Mock permission denied error (use generic message without function name
      // to avoid matching isMissingRpc check which triggers on 'clear_all_user_data')
      (mockSupabaseClient.rpc as jest.Mock).mockResolvedValue({
        data: null,
        error: { code: '42501', message: 'permission denied for schema public' },
      });

      // No fallback to manual deletion - atomicity requires RPC
      await expect(dataStore.clearAllUserData()).rejects.toThrow(NetworkError);
      await expect(dataStore.clearAllUserData()).rejects.toThrow('Permission denied');
    });
    ...
    it('should throw on unknown RPC error', async () => {
      // Mock unknown error
      (mockSupabaseClient.rpc as jest.Mock).mockResolvedValue({
        data: null,
