...
## CI/CD System and Code Review Workflows

### Overview

The project uses a comprehensive GitHub Actions CI/CD system with **5 primary workflows**:

1. **`ci.yml`** - Main CI pipeline (runs on every push/PR)
2. **`claude-code-review.yml`** - Standard code review
3. **`claude-custom-review.yml`** - Specialized keyword-driven reviews ⭐
4. **`test-guards.yml`** - Test quality enforcement
5. **`full-test-suite.yml`** - Complete test suite runs

### Keyword-Driven Review System

The `claude-custom-review.yml` workflow supports **7 specialized review types** that can be triggered via PR labels or manual dispatch:

#### Available Review Types

1. **`review:security`** - Security-focused review
   - Authentication & authorization vulnerabilities
   - Data protection and encryption
   - Input validation (XSS, injection)
   - Browser security (CSP, CORS, cookies)
   - Privacy & GDPR compliance

2. **`review:performance`** - Performance optimization review
...
3. **`review:architecture`** - Architecture quality review
...
4. **`review:testing`** - Testing quality review
...
5. **`review:test-suite-health`** - Comprehensive test suite health assessment ⭐
   - **Analyzes the ENTIRE test suite**, not just PR changes
   - Provides detailed health report with A-F grade
   - Covers: metrics, organization, quality indicators, flaky tests, isolation, mock quality, performance, coverage gaps, maintenance, best practices
   - Outputs: executive summary, detailed findings with status indicators (✅/⚠️/❌), prioritized recommendations, metrics dashboard

6. **`review:documentation`** - Documentation quality review
   - JSDoc completeness
   - README updates
   - API documentation
   - Inline comments quality

7. **`review:accessibility`** - Accessibility review
   - Semantic HTML
   - ARIA attributes
   - Keyboard navigation
...
**Automatic (Label-Based):**
```bash
# Single review type
gh pr edit <PR_NUMBER> --add-label "review:security"

# Multiple reviews in parallel
gh pr edit <PR_NUMBER> --add-label "review:security" --add-label "review:performance"
```

**Manual (Workflow Dispatch):**
```bash
# Predefined review type
...
gh workflow run claude-custom-review.yml \
  ...
  -f custom_prompt="Review IndexedDB schema changes for data consistency and migration safety" \
  -f pr_number=35
```
...
| Test suite improvements | `test-suite-health`, `testing` |

### Available Labels

All CI review labels:
- `code-review` - Standard Claude Code review
- `review:security` - Security-focused review
- `review:performance` - Performance review
- `review:architecture` - Architecture review
- `review:testing` - Testing review
- `review:test-suite-health` - Full test suite health assessment
- `review:documentation` - Documentation review
- `review:accessibility` - Accessibility review

### Review Output

Reviews post structured PR comments including:
- Issue severity/priority (Critical, High, Medium, Low)
- Specific code locations (file paths and line numbers)
- Suggested fixes with code examples
- Best practice recommendations

For `test-suite-health`, output includes:
- Executive summary with overall grade (A-F)
- Detailed findings with status indicators
- Prioritized recommendations (Critical/High/Medium/Low)
- Metrics dashboard (total tests, pass rate, execution time, coverage, flaky tests)

### Documentation

See `.github/workflows/README.md` for complete workflow documentation and examples.
