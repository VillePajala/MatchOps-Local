...
- [ ] **`get_user_consent` returns stale 'granted'** (#371) — **RPC fix is LIVE**: prod was hotfixed 2026-06-10 (`fix_consent_rpc_ordering`, status-aware + `policy_version DESC`); repo synced via migration 029 (PR #422). **Remaining**: regenerate `src/types/supabase.ts` (missing `user_consents.status`), verify in-app, close #371; optional consent hardening notes (uuid id, month regex) from #374/#375.
...
- [x] **Timer pauses on backgrounding and silently stays paused** (user-reported incident, 2026-06-15) — ✅ **DONE 2026-06-16** (PR #427): in-session background→foreground path (separate from CR-C1's reload path). Old code paused on hide and relied on reading a `wasRunning` marker back from IndexedDB to resume; that read fails on mobile (fire-and-forget write never flushes before freeze, or a no-`wasRunning` tick-save overwrites it) → timer stayed paused, match time lost. Fix: never pause on backgrounding — the precision timer is wall-clock based, so on return re-anchor to `Date.now()` truth and keep running (`usePrecisionTimer.reanchor`); pausing happens only on the explicit control. Follow-ups below.
  - [ ] **>5min auto-resume** — `useAppResume` force-reloads after >5min background (blank-screen recovery); CR-C1 then leaves the timer accurate-but-paused awaiting a tap. To honor "never paused" end-to-end, auto-resume after that reload when a `wasRunning` marker matched the loaded game (touches CR-C1 boot + undo baseline — deliberate separate change). Alt: raise `forceReloadTime`.
  - [ ] **Dead-code cleanup** — `PAUSE_TIMER_FOR_HIDDEN` + `RESTORE_TIMER_STATE` reducer actions and `useTimerRestore` export are now orphaned in production (still defined + tested). Remove across reducer union, undo/redo action-classification list, and 3 test suites.
  - [ ] ⚠️ **On-device QA** — start game → lock phone <5min → reopen → clock kept counting (no tap needed); and the >5min case (still needs a tap until the follow-up above).
...
- [ ] **Test-suite cumulative memory leak → CI "Test" job OOM** (found 2026-06-17, PR #431) — the full Jest run (~230 suites, ~4,860 tests) accumulates heap across suite files and never releases it; a single-process (`--runInBand`) run climbs steadily and exhausts even an 8 GB cap. CI normally squeaks under the 4 GB ceiling with `maxWorkers: 2`, but it's balanced on a knife's edge: **adding one test reshuffles suite→worker scheduling and tips a worker into OOM** (exit 134). Mitigations that did NOT work: raising `--max-old-space-size` to 6 GB/8 GB; `workerIdleMemoryLimit` at 1 GB and 512 MB (recycles too late / leak too aggressive). Smoking gun: repeated `MaxListenersExceededWarning: 11 uncaughtException listeners added to process` — listeners (and their retained closures) piling up across suites, likely from a setup file or an adapter/jsdom teardown that doesn't remove handlers. **Cost we're paying now:** can't freely add tests; the Unknown-scorer goal **edit round-trip test was omitted** from PR #431 (the edit-form *fix* shipped; only its automated test was dropped — see the NOTE in `GoalLogModal.test.tsx`). Fix: find the leaking listener/closure (audit `src/setupTests.mjs`, `tests/utils/*.js`, `indexedDbKvAdapter` teardown, jsdom global handlers), ensure per-suite cleanup removes what it adds, then re-add the omitted test.
