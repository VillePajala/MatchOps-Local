...
- [x] **Timer pauses on backgrounding and silently stays paused** (user-reported incident, 2026-06-15) — ✅ **DONE 2026-06-16** (PR #427): in-session background→foreground path (separate from CR-C1's reload path). Old code paused on hide and relied on reading a `wasRunning` marker back from IndexedDB to resume; that read fails on mobile (fire-and-forget write never flushes before freeze, or a no-`wasRunning` tick-save overwrites it) → timer stayed paused, match time lost. Fix: never pause on backgrounding — the precision timer is wall-clock based, so on return re-anchor to `Date.now()` truth and keep running (`usePrecisionTimer.reanchor`); pausing happens only on the explicit control. Follow-ups below.
  - [x] **>5min auto-resume** — ✅ **DONE 2026-06-19**. The boot path already folded background time into the clock (capped at the period boundary) but left the timer paused; now, when the timer `wasRunning` at hide and the game is still mid-period (corrected clock below the boundary), it auto-dispatches `RESUME_GAME` after load so the clock continues without a tap. If the period ended during the gap (clock hit the boundary) it stays paused so the user taps to acknowledge — we never silently auto-advance periods. Also fixed a latent bug surfaced along the way: `RESUME_GAME` was uncategorized in the history wrapper (logged a console.error on every manual resume too) and missing from the action-validation reference list — now classified NO_HISTORY. **On-device QA of the >5min case still recommended.**
  ...
  - [ ] ⚠️ **On-device QA** — start game → lock phone <5min → reopen → clock kept counting (no tap needed); and the >5min case → reopen → clock should now auto-continue (no tap) while still mid-period, or sit paused at the period boundary if the period elapsed during the lock.
...
- [ ] **🔺 Investigate: in-app update banner shows the SAME text after every deploy** (reported 2026-06-19, **high priority**) — the release-notes system (PR #433) is meant to surface fresh per-release notes, but the banner text appears unchanged across master merges. Likely contributors to rule in/out: (1) most recent PRs carried `skip-release-notes` (internal) so `release-notes.json` legitimately didn't change — confirm whether that fully explains it; (2) verify `generate-changelog.mjs` actually runs in the Vercel build and `public/changelog.json` is regenerated per deploy; (3) verify the deployed `changelog.json` reflects the top `release-notes.json` entry (not a stale committed copy); (4) check the service worker / HTTP cache isn't serving a stale `changelog.json` despite the `?t=` cache-bust; (5) reconsider whether the guard should *require a NEW top entry* (not just any edit) for non-skip PRs. The guard "forces an edit or a skip label" is not the same as "forces fresh user-facing text" — that gap may be the real issue.
