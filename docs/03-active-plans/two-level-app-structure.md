...
| **1.1 Auto-resume guardrail** | Session key (planner pattern): live match open → app launch lands IN the match. Ships FIRST so game-day speed can never regress unnoticed. | Test: backgrounded live game + relaunch → match view, no extra tap. |
...
| **3.1 Match menu shrink** | Match menu to ~6 items (Ottelun tiedot, Arvioi pelaajat, Otteluraportti, Ottelun tilastot + "Joukkueen tilastot →" link, Tallenna/Tallenna nimellä, ← Koti). Hardware back mirrors ← Koti. | Reachability table (§2) holds exactly; menu tests updated. |
