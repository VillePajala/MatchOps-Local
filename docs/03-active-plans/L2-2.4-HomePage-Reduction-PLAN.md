...
Next (2.4.9)
- Eliminate remaining direct modal setters from `GameContainer`, `ControlBar`, and `ModalManager` by consuming the reducer helper API everywhere (including save-before-new + instructional overlays).
- Collapse ModalManager props around reducer state so modal-specific state machines (team roster manager, load game confirmations) sit on top of the centralized reducer.
- Add an integration test that drives ControlBar shortcuts and ensures reducer-backed anti-flash guards fire exactly once per intent.
