...
- [x] 2.5 Edge-case regression tests & memoization enforcement
...
Next (Final Integration — HomePage orchestrator cleanup)
- Wire `HomePage.tsx` to consume `useGameOrchestration`, `GameContainer`, and `ModalManager`, remove the remaining inline JSX/handlers, and shrink the file to an orchestrator-only shell (<200 lines).
- Manual verification: Load/New game flows, ControlBar shortcuts, roster edits, and first-game guide should continue to work once the orchestrator hands off everything to the extracted containers.
