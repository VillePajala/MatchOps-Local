...
Next (2.4.7)
- Begin extracting FieldContainer interaction view-models (actions vs. read-only state) so the component can accept cohesive objects instead of dozens of handlers.
- Introduce the first `modalReducer` slice for Load/New modals using the new grouped contexts.
- Keep trimming adapter fallbacks (e.g., remove legacy PlayerBar/GameInfo props entirely once GameContainer is wired into HomePage).
