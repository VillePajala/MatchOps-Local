...
Next (2.4.4)
- Continue HomePage reduction by grouping PlayerBar/GameInfo props via VM, then remove duplicated props.
- Unify debug flags and trim remaining noisy logs.
- Consider grouping container props (`FieldContainer`) into sub‑objects to reduce interface size (tracked as tech‑debt).
