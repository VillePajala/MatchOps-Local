# Layer 2 — Step 2.7: useGameOrchestration Cleanup

**Status**: ✅ **COMPLETE** (PR #103 ready for merge)
**Priority**: 🟢 **DONE** — Cleanup completed
**Actual Effort**: ~4 hours (4 PRs merged to integration branch)
**Prerequisites**: Step 2.6 (hook extractions) ✅ Complete
...
```
master
  └── integration/2.7-orchestration-cleanup   (integration branch)
        ├── refactor/2.7.1-cleanup-data-management
        ├── refactor/2.7.2-cleanup-session-field
        ├── refactor/2.7.3-cleanup-persistence-timer
        └── refactor/2.7.4-final-cleanup
```
...
3. **PRs target integration branch** (not master):
   - PR 1: `refactor/2.7.1-cleanup-data-management` → `integration/2.7-orchestration-cleanup`
   - PR 2: `refactor/2.7.2-cleanup-session-field` → `integration/2.7-orchestration-cleanup`
   - PR 3: `refactor/2.7.3-cleanup-persistence-timer` → `integration/2.7-orchestration-cleanup`
   - PR 4: `refactor/2.7.4-final-cleanup` → `integration/2.7-orchestration-cleanup`

4. **Final PR to master**:
   - After all 4 PRs merged to integration
   - Final PR: `integration/2.7-orchestration-cleanup` → `master`
...
### After Final PR (Integration → Master)
1. Full test suite
...
- [ ] `useGameOrchestration.ts` ≤ 300 lines
...
- ✅ Step 2.6.1: useGameDataManagement (98% test coverage)
- ✅ Step 2.6.2: useGameSessionCoordination (97% test coverage)
- ✅ Step 2.6.3: useFieldCoordination (99% test coverage)
- ✅ Step 2.6.4: useGamePersistence (78% test coverage)
- ✅ Step 2.6.5: useTimerManagement (100% test coverage)
- ✅ Step 2.6.6: useModalOrchestration (61% test coverage)
...
1. **Step 2.8: Modal Architecture Fix** — Fix 76-parameter anti-pattern in useModalOrchestration
2. See [L2-2.8-Modal-Architecture-Fix-PLAN.md](./L2-2.8-Modal-Architecture-Fix-PLAN.md)
...
- PR #99: Removed data management duplicates (unused Season/Tournament imports)
- PR #100: Removed commented-out code blocks (-103 lines)
- PR #101: Cleaned up TODO/development comments (-72 lines)
- PR #102: Consolidated imports (-22 lines)
- PR #103: Final merge to master (pending user testing)

**Result**: useGameOrchestration.ts reduced from 2,199 → 1,998 lines (-201 lines, 9% reduction)

**Note**: The original 200-300 line target was unrealistic. Analysis showed the remaining ~2,000 lines contain genuine orchestration logic:
- 47 imports (necessary dependencies)
- 41 lines initialState config
- 18 useState declarations (hook coordination)
- 10+ useEffect for cross-hook coordination
- View-model building and prop assembly

The real architectural issue discovered was the **76-parameter anti-pattern in useModalOrchestration**, which is addressed in Step 2.8.

---

**Created**: 2025-12-04
**Updated**: 2025-12-04 (Marked complete, added completion summary)
