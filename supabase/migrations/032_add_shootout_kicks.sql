-- ============================================================================
-- 032: Penalty-shootout kicks (Overtime + Penalties feature, PR 2)
-- ============================================================================
-- Adds a per-kick penalty-shootout log to games. The shootout result (tally +
-- winner) is derived in the app from this list; the column just persists it.
-- Stored as JSONB so future per-kick fields (goalkeeperId, missType) are
-- additive with no further migration.
--
-- Two parts:
--   1. ADD COLUMN games.shootout_kicks jsonb (nullable; existing rows = NULL).
...
--
-- The function is recreated VERBATIM from migration 030 with ONLY one added
-- line ("shootout_kicks = EXCLUDED.shootout_kicks"). Before applying to PROD,
-- diff prod's current definition against migration 030's base (per CLAUDE.md).
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS shootout_kicks jsonb;
...
BEGIN
  ...
  -- Upsert game (includes columns from migrations 021, 022 and 032)
