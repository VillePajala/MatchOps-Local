-- ============================================================================
-- 047: Which pitch at the venue
-- ============================================================================
-- games.game_location was carrying two different facts in one string: the
-- VENUE ("Kimpisen kentta") and the PITCH within it ("TN 2"). That broke both
-- of them at once.
--
...
--      stay NULL - nothing is parsed out of the old strings, because "TN 2" and
--      "kentta 3" and "Field 1" are not a grammar worth guessing at, and a
--      wrong split is harder to notice than an empty box.
--   2. Recreate save_game_with_relations so the ON CONFLICT DO UPDATE path
--      writes it. The INSERT path picks it up on its own
--      (jsonb_populate_record is column-agnostic), which is exactly the half
--      that made migration 043 look complete while every UPDATE dropped the
--      value.
--
-- The function below is migration 045's, verbatim, with ONE line added
-- ("field_number = EXCLUDED.field_number"). Before PROD, diff prod's live
-- definition against 045 (per CLAUDE.md) and apply only when the sole
-- differences are comments and whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS field_number text;

COMMENT ON COLUMN games.field_number IS
  'Which pitch at the venue ("TN 2"); NULL when unspecified. The venue itself is game_location.';
...
BEGIN
  ...
  -- Upsert game (includes columns from migrations 021, 022, 032, 035, 039, 044, 045 and 047)
