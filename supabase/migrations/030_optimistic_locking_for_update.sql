-- ============================================================================
-- 030: Make optimistic locking actually work (CR-M3 #1)
-- ============================================================================
-- Two fixes to save_game_with_relations, both verified against a staging DB:
--
-- 1. TOCTOU race — the version was read with a plain SELECT, then checked and
--    updated. A concurrent save could read the same version, both pass the
--    check, and both write. Fix: `FOR UPDATE` locks the game row for the
--    transaction, so a concurrent save blocks until this one commits and then
--    correctly conflicts.
--
-- 2. Conflicts never reached the client — the conflict was raised with ERRCODE
--    'serialization_failure' (SQLSTATE 40001). PostgREST treats 40001 as a
--    transient serialization failure and AUTO-RETRIES it; since an optimistic-
--    lock conflict fails identically every retry, the request retried until the
--    gateway timed out (HTTP 504). The user never saw "modified elsewhere".
--    Fix: raise with SQLSTATE 'PT409' — PostgREST maps PTxxx to HTTP xxx and
--    does NOT retry it, so the conflict returns immediately as HTTP 409.
--    (App-side detection updated to match: src/datastore/SupabaseDataStore.ts.)
--
-- Recreated verbatim from the prior definition with ONLY these two changes.
-- ============================================================================
