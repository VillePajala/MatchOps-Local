-- ============================================================================
-- 039: Friendly / practice matches (harjoitusottelut)
-- ============================================================================
-- Adds an explicit "this game is a friendly / practice match" flag to games.
-- Friendlies are recorded like any game but are excluded from competitive
-- (Overall / per-player) stat totals by default; the app can fold them back in
-- via an "Include friendly matches" toggle. The flag is explicit (set by the
-- coach) - it is NOT inferred from "no season and no tournament", so a
-- competitive game the coach forgot to tag is never dropped from the record.
--
-- Two parts (mirrors migration 035):
--   1. ADD COLUMN games.is_friendly boolean NOT NULL DEFAULT false. Existing
--      rows become false (not friendly), so nothing changes retroactively.
--   2. Recreate save_game_with_relations so the ON CONFLICT DO UPDATE SET path
--      writes the new column (needed for reclassifying an existing game). The
--      INSERT path already covers it (jsonb_populate_record maps it from p_game
--      once the column exists).
--
-- The function is recreated VERBATIM from migration 035 with ONLY one added
-- line ("is_friendly = EXCLUDED.is_friendly"). Before applying to PROD, diff
-- prod's current definition against migration 035's base (per CLAUDE.md) - apply
-- only when the sole differences are comments/whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS is_friendly boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  -- Upsert game (includes columns from migrations 021, 022, 032, 035 and 039)
