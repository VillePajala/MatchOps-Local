...
-- 045: Game captain
-- ============================================================================
-- Records who wore the armband, per game. Captaincy rotates in youth football,
-- so this belongs to the match and not to the team, and being asked to lead
-- once is worth having on the record.
--
-- Two parts, the shape migration 039 established:
--   1. ADD COLUMN games.captain_id text, nullable, no default. Existing rows
--      stay NULL: no game retroactively acquires a captain.
--   2. Recreate save_game_with_relations so the ON CONFLICT DO UPDATE path
--      writes it. The INSERT path picks it up on its own (jsonb_populate_record
--      is column-agnostic), which is exactly the half that made migration 043
--      look complete while every UPDATE silently dropped the value.
--
-- No foreign key to players on purpose: a game keeps its record even after the
-- roster changes, the same way scorer_id on game_events does.
--
-- The function below is migration 044's, verbatim, with ONE line added
-- ("captain_id = EXCLUDED.captain_id"). Before PROD, diff prod's live
-- definition against 044 (per CLAUDE.md) and apply only when the sole
-- differences are comments and whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS captain_id text;

COMMENT ON COLUMN games.captain_id IS
  'Player id who captained this game; NULL when no captain was named.';
...
BEGIN
  ...
  -- Upsert game (includes columns from migrations 021, 022, 032, 035, 039, 044 and 045)
