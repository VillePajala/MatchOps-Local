-- ============================================================================
-- 049: The venue has two names, and they are different facts
-- ============================================================================
-- OpenStreetMap knows buildings by their real names, not their sponsors. The
-- hall a coach calls "Mitta-Keittiot Areena" is on the map as a plain jaahalli
-- at "Pihlajavedentie 1, Savonlinna" - so the string that FINDS the place and
-- the string that NAMES it to a parent are two different things, and one
-- column cannot hold both.
--
-- game_location keeps what the coach calls it. This column keeps what the
-- lookup returned. That separation is what lets a picked venue be renamed
-- without losing its pin: the old rule cleared the coordinates the instant the
-- text was edited, because a pin that silently contradicts the words beside it
-- is worse than no pin. Showing this address under the field answers that
-- objection instead of forbidding the edit - the name and the pinned place are
-- both visible, so they cannot drift apart unnoticed.
--
-- NULL for every existing row and for any venue typed by hand. The name is
-- still the location; this is an optional upgrade to it, and the app must keep
-- working for a coach who never picks a suggestion.
--
-- The function below is migration 048's, verbatim, with ONE line added to the
-- ON CONFLICT DO UPDATE list. Before PROD, diff prod's live definition against
-- 048 (per CLAUDE.md) and apply only when the sole differences are comments
-- and whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS location_address text;

COMMENT ON COLUMN games.location_address IS
  'The venue address as the lookup returned it; NULL when the location was typed rather than picked. game_location holds what the coach calls the place.';
