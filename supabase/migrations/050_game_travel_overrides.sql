-- ============================================================================
-- 050: What THIS match's journey needs, when the club default does not fit
-- ============================================================================
-- The next-match card works out when to leave: kick-off, minus the time you
-- must already BE at the ground, minus the drive. Both of those subtrahends
-- have a club-wide default in settings, and both are wrong often enough that a
-- single match has to be able to say otherwise.
--
--   arrival_buffer_minutes - a cup tie or a tournament may ask for an hour
--     where the league asks for thirty. Without a per-match figure the coach
--     changes the default for one fixture and forgets to change it back, which
--     is the forgetting this app keeps having to design around.
--
--   travel_minutes - how long the drive REALLY took, once the coach has said
--     so. It replaces a straight-line estimate that knows nothing about the
--     lake you drive around, and the venue book carries it forward to every
--     later match at the same place. That is the whole reason this is stored
--     per match rather than recomputed: a measured number beats a guess, and
--     it only exists if somebody writes it down.
--
-- NULL for every existing row and for every match nobody has adjusted. Both are
-- overrides; the app must work exactly as before when neither is set.
--
-- The function below is migration 049's, verbatim, with TWO lines added to the
-- ON CONFLICT DO UPDATE list. Before PROD, diff prod's live definition against
-- 049 (per CLAUDE.md) and apply only when the sole differences are comments
-- and whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS arrival_buffer_minutes integer;
ALTER TABLE games ADD COLUMN IF NOT EXISTS travel_minutes integer;

COMMENT ON COLUMN games.arrival_buffer_minutes IS
  'Minutes to be at the ground before THIS kick-off, overriding the club default; NULL means use the default.';
COMMENT ON COLUMN games.travel_minutes IS
  'Measured drive time to this venue in minutes; NULL means the app estimates it from the distance.';
...
BEGIN
  ...
  ON CONFLICT (user_id, id) DO UPDATE SET
    ...
    location_address = EXCLUDED.location_address,       -- Added in migration 049
    arrival_buffer_minutes = EXCLUDED.arrival_buffer_minutes, -- Added in migration 050
    travel_minutes = EXCLUDED.travel_minutes,           -- Added in migration 050
