-- ============================================================================
-- 035: Per-player positions played (Line-up feature, cloud sync)
-- ============================================================================
-- Adds the post-game "which position(s) did each player play" map to games.
-- Stored as JSONB (an id-keyed map: player id -> array of position ids), the
-- same non-schema-migrating pattern as assessment slider_values. The app reads
-- it back as AppState.playerPositions; a missing/NULL value = no positions
-- recorded (legacy rows).
--
-- Two parts (mirrors migration 032):
--   1. ADD COLUMN games.player_positions jsonb (nullable; existing rows = NULL).
--   2. Recreate save_game_with_relations so the ON CONFLICT DO UPDATE SET path
--      writes the new column. The INSERT path already covers it automatically
--      (jsonb_populate_record maps it from p_game once the column exists).
--
-- The function is recreated VERBATIM from migration 032 with ONLY one added
-- line ("player_positions = EXCLUDED.player_positions"). Before applying to
-- PROD, diff prod's current definition against migration 032's base (per
-- CLAUDE.md) - apply only when the sole differences are comments/whitespace.
-- ============================================================================

ALTER TABLE games ADD COLUMN IF NOT EXISTS player_positions jsonb;
...
BEGIN
  ...
  -- Upsert game (includes columns from migrations 021, 022, 032 and 035)
