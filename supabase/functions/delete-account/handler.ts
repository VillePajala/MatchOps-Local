/**
 * Delete Account Edge Function — request handler
 *
 ...
 *
 * The handler is built via createHandler(deps) so tests can inject mock
 * Supabase clients and exercise THIS code (not a re-implementation).
 * index.ts wires in the real clients and calls Deno.serve.
 *
...
export const RATE_LIMIT_MAX_REQUESTS = 5; // Max 5 requests per minute per IP (stricter for destructive operation)

// Minimal structural types for the client capabilities the handler uses.
// Keeps tests decoupled from the full supabase-js client surface.
...
export interface Deps {
  ...
  createUserClient(url: string, anonKey: string, jwt: string): UserClient;
}
...
export function createHandler(deps: Deps): (req: Request) => Promise<Response> {
  return async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      const supabaseUser = deps.createUserClient(supabaseUrl, anonKey, jwt);
