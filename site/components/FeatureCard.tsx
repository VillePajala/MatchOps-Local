...
export default function FeatureCard({ icon, title, description, highlights, variant = 'default' }: FeatureCardProps) {
  ...
  return (
    <div className="bg-slate-900/70 rounded-xl border border-slate-700/40 shadow-card hover:shadow-card-hover transition-shadow p-2 md:p-4 overflow-hidden">
      <div className="h-full rounded-lg p-4 md:p-6 bg-gradient-to-br from-slate-600/40 to-slate-800/30 hover:from-slate-600/50 hover:to-slate-800/40 transition-all border border-slate-700/40">
        <div className="text-primary text-2xl md:text-4xl mb-3 md:mb-4">
          {icon}
        </div>
        <h3 className="text-sm md:text-xl font-bold mb-2 text-white tracking-wide break-words hyphens-auto">
          {title}
        </h3>
        {description && (
